...
public class Deck {
	...
	public void riffleShuffle() {
		// Divide the deck into two unequal halves
		int max1 = 50;
		int min1 = 0;
		int middle = gen.nextInt((max1 - min1) + 1) + min1;
		Card[] top = new Card[middle];
		Card[] bottom = new Card[52 - middle];

		for (int i = 0; i < middle; i++) {
			top[i] = cards.get(i);
		}
		for (int i = 0; i < 52; i++) {
			bottom[i] = cards.get(middle + i);
		}
		ArrayList<Card> newCards = new ArrayList<Card>();
		// Interleave the cards at random intervals from 1-4
		for (int i = 0; i < 52; i++) {
			int max2 = 4;
			int min2 = 1;
			int topCounter = 0;
			int bottomCounter = 0;
			int interval = gen.nextInt((max2 - min2) + 1) + min2;
			for (int a = 0; a < interval; a++) {
				newCards.add(top[ topCounter ]);
				topCounter++;
			}
			int middle3 = gen.nextInt((max2 - min2) + 1) + min2;
			for (int b = 0; b < middle3; b++) {
				newCards.add(bottom[bottomCounter]);
				bottomCounter++;
			}
			cards = newCards;
		}
